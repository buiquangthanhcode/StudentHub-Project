...
class GeneralProjectBloc extends Bloc<GeneralProjectEvent, GeneralProjectState> {
  ...
  FutureOr<void> _onGetAllData(GetAllDataEvent event, Emitter<GeneralProjectState> emit) async {
    try {
      ...
      ResponseAPI result = await _allProjectsService.getAllProjects(event.page == 0 ? 1 : event.page, event.perPage);
      ...
      if (result.statusCode! < 300 || result.statusCode == 404) {
        emit(state.update(projectList: event.page == 0 ? result.data : data, isLoading: !state.isLoading));
      } else {
  ...
  FutureOr<void> _onGetProjectDetail(GetProjectDetail event, Emitter<GeneralProjectState> emit) async {
    try {
      ...
      logger.e("Unexpect error-> $e");
      ResponseAPI message = e as ResponseAPI;
      SnackBarService.showSnackBar(content: handleFormatMessage(message.data.toString()), status: StatusSnackBar.error);
    } finally {
      EasyLoading.dismiss();
    }
  }
